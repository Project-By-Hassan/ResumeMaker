...
Bug fix: HTML built with pure string concat, no f-strings at all,
unsafe_allow_html=True on the single st.markdown call.
"""
...
def render_progress_bar():
    ...
    # choose color + label based on pct
    if pct == 100:
        bar_color = "#10b981"
    ...
    for name, done in sections:
        ...
        dots_html += (
            ...
            'gap:3px;flex:1;min-width:46px;max-width:68px;">'
            '<div style="width:26px;height:26px;border-radius:50%;background:' + dot_bg + ';'
            'display:flex;align-items:center;justify-content:center;'
            'font-size:0.72rem;color:white;font-weight:700;">' + mark + '</div>'
            '<div style="font-size:0.59rem;color:' + txt_clr + ';font-weight:500;'
            'text-align:center;line-height:1.3;">' + name + '</div>'
            '</div>'
        )
    ...
    html = (
        ...
        'padding:1rem 1.3rem;margin:0 0 1.2rem 0;'
        'box-shadow:0 2px 10px rgba(99,102,241,0.06);">'

        '<div style="display:flex;justify-content:space-between;'
        'align-items:center;margin-bottom:8px;">'
        '<span style="font-size:0.84rem;font-weight:700;color:#374151;">' + label + '</span>'
        ...
        '<div style="display:flex;justify-content:space-between;'
        'align-items:flex-start;flex-wrap:nowrap;gap:2px;">'
        + dots_html +
        '</div>'
        '</div>'
    )
